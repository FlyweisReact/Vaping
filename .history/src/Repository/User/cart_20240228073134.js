...
export const successOrderForPaypal = async (paymentId  ,PayerID, setIsVerified) => {
  try {
    const res = await axios.get(
      `${BaseUrl}api/v1/user/successOrderForPaypal?paymentId=${paymentId}&PayerID=6554&amount=1`,
      {
        headers: { Authorization: `Bearer ${localStorage.getItem("Token")}` },
      }
    );
