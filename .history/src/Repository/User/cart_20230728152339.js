...
const deleteProductCart = async (payload) => {
  try {
    const response = await axios.put(
      `${BaseUrl}https://krish-vapes-backend.vercel.app/api/v1/user/deleteProductfromCart/64c389c2a21c8f4d8b2ab15c`,
      {
        cartProductId: payload,
      },
      {
        headers: {
          Authorization:
            "Bearer eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpZCI6IjY0YzM2Njg4YzY0NjM3OGZlODczMzc2YSIsImlhdCI6MTY5MDUyOTc3NiwiZXhwIjoxNjkwNzg4OTc2fQ.Lv5LFkFJGaakIRKejXGQXgHO-dTCB8hoy5PLPiN1zN8",
        },
      }
    );
    const msg = response.data.message;
...
const updateQuantityCart = async (payload) => {
  try {
    const response = await axios.put(
