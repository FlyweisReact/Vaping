...
import { FilterProducts, getProductAuth } from "../../Repository/User/Product";
...
const Homepage = () => {
  ...
  const [query, setQuery] = useState(
    isLoggedIn === true ? "getNewArrivalByToken" : "NewArrival"
  );
  ...
  const getAuthProducts = async () => {
    setProductLoading(true);
    try {
      const response = await getProductAuth(query);
      setProducts(response);
      setProductLoading(false);
    } catch {
      setProductLoading(false);
          ...
          <div className="Container">
            ...
            {isLoggedIn === true ? (
              <div className="switch_btn">
                <button
                  className={query === "getNewArrivalByToken" ? "active" : ""}
                  onClick={() => setQuery("getNewArrivalByToken")}
                >
                  New Arrivals
                </button>
                <button
                  className={query === "BestSellerByToken" ? "active" : ""}
                  onClick={() => setQuery("BestSellerByToken")}
                >
                  Bestseller
                </button>
                <button
                  className={query === "getOnSaleByToken" ? "active" : ""}
                  onClick={() => setQuery("getOnSaleByToken")}
                >
                  On Sale
                </button>
              </div>
...
{/* New Update */}
            ...
            <Product products={searchD} loading={productLoading} productFetching={productFetching}  />
