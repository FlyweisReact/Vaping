...
const handlePayment = async () => {
  ...
  const response = await fetch('/create-payment-intent', {
    ...
    body: JSON.stringify({ amount: 1000, currency: 'usd' }), // Adjust the amount and currency accordingly
  });
  ...
  const result = await stripe.confirmCardPayment(clientSecret, {
    payment_method: {
      card: elements.getElement(CardElement), // If using Stripe Elements for card collection
    },
  });
