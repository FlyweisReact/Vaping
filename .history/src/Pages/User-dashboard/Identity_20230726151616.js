...
const Identity = () => {
  ...
  const [email, setEmail] = useState(Details?.firstName);
  const [password, setPassword] = useState(Details?.firstName);
  const [dob, setDob] = useState(Details?.firstName);
  const [courtesyTitle, setCourtesyTitle] = useState(Details?.firstName);
